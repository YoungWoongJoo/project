...
<html>
  <head>
		...
		function fn_delete()
    {
        ...
        var owner = $("<input type='hidden' name='warehouse_owner' value="+td.eq(2).text()+">");
        var code = $("<input type='hidden' name='warehouse_code' value="+td.eq(3).text()+">");
        var region = $("<input type='hidden' name='warehouse_region' value="+td.eq(4).text()+">");
        var rating = $("<input type='hidden' name='warehouse_rating' value="+td.eq(5).text()+">");
  ...
  <body>
    ...
    <table border="1">
        <thead>
            <th>선택</th><th>창고명</th><th>창고주명</th><th>코드번호</th><th>지역구분</th><th>등급구분</th>
        </thead>
        <c:choose>
